...
// swimmingDistance возвращает дистанцию(в километрах), которую преодолел пользователь за время тренировки.
//
// Параметры:
//
// lengthPool int — длина басейна , countPool - число проплытых бассейнов.
func swimmingDistance(lengthPool, countPool int) float64 {
	return float64(lengthPool) * float64(countPool)
}
...
// Я НЕ ЗНАЮ ПОЧЕМУ, НО ЕСЛИ ЭТА ФУНКЦИЯ СТОИТ НА СВОЕМ ИЗНАЧАЛЬНОМ МЕСТЕ - ОНА НЕ АВТОКОМПЛИТИТСЯ И ЕЁ НЕ ВИДНО, НУ ПРЯМО НИКАК
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	calculate := (runningCaloriesMeanSpeedMultiplier * averageSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
	return calculate
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	avgSpeedMsec := meanSpeed(action, duration) * kmhInMsec
	avgSpeedPow := math.Pow(avgSpeedMsec, 2)
	weightK1 := weight * walkingCaloriesWeightMultiplier
	weightK2 := weight * walkingSpeedHeightMultiplier

	result := (weightK1 + avgSpeedPow/(height/cmInM)*weightK2) * duration * minInH
	return result
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	result := (meanSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return result
}
